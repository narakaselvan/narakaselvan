...
# ==========================================
def show_agricultural_analytics():
    st.markdown(
        """
    ...
    def fetch_analytical_data(area_prefix):
        ...
        try:
            ...
            # FIXED: Added `MAX(assignment__id::VARCHAR) AS assignment_id` inside MainData CTE
            sql = f"""
            WITH MainData AS (
                SELECT 
    ...
    r1c1.text_input("📍 Island", value="Sri Lanka", disabled=True, key="ag_isl")
    
    provs = sorted([x for x in set(df_filt['province_name']) if x != 'Unknown'])
    sel_prov = r1c2.selectbox("📍 Province", ["All"] + provs, key="ag_prov_s") if len(prefix) < 1 else r1c2.text_input("📍 Province", df_filt['province_name'].iloc[0] if not df_filt.empty else "N/A", disabled=True, key="ag_prov_l")
    ...
    sel_dist = r1c3.selectbox("📍 District", ["All"] + dists, key="ag_dist_s") if len(prefix) < 2 else r1c3.text_input("📍 District", df_filt['district_name'].iloc[0] if not df_filt.empty else "N/A", disabled=True, key="ag_dist_l")
    ...
    sel_div = r1c4.selectbox("📍 Division", ["All"] + divs, key="ag_div_s") if len(prefix) < 4 else r1c4.text_input("📍 Division", df_filt['division_name'].iloc[0] if not df_filt.empty else "N/A", disabled=True, key="ag_div_l")
    ...
    sel_gn = r2c1.selectbox("📍 GN Division", ["All"] + gns, key="ag_gn_s") if len(prefix) < 7 else r2c1.text_input("📍 GN Division", df_filt['gndivision_name'].iloc[0] if not df_filt.empty else "N/A", disabled=True, key="ag_gn_l")
    if sel_gn != "All" and len(prefix) < 7: df_filt = df_filt[df_filt['gndivision_name'] == sel_gn]

    blks = sorted(list(set(df_filt['Block'])))
    sel_blk = r2c2.selectbox("🏢 Block", ["All"] + blks, key="ag_blk_s")
